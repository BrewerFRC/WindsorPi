...
pressedY = False 	## Detect Y button on rising edge.
pressedA = False 	## Detect A button on rising edge.
pressedX = False
pressedB = False 	## Used to apply actions to falling edge of B button.
pressedBumpers = False 	## Detect both bumpers pressed on rising edge.
pressedRT = False 	## Detect Right Trigger on rising edge.
throwInProgress = False ## State that a throw in in progress.
throwSpeedLock = False  ## Lock thrower speed in place.
pressedThumb = False    ## Detect thumbstick on rising edge.
pressedRJ = False       ## Detect a rising edge on the right joystick
...
try:
	...
	## starting the robot and if the controller is connected.
        if isStarted and j.connected(): 
		    ...
		    ## Robot will respond only if joystick is more than 20% pressed to avoid accidental activation.
                    if abs(j.leftX()) <= .10 and abs(j.leftY()) <= .10:
                        ...
                        dt.drive(j.leftX() * .5, j.leftY() * .5) ## Drive at 50% power. Turn at 50% power.
	    ...
	    ## Control: Left & Right Bumpers at the same time.
	    if j.leftBumper() and j.rightBumper():
                if not pressedBumpers:
                    ArmDisabled = not ArmDisabled
                    pressedBumpers = True
            else:
                pressedBumpers = False
            
            ## Start Intake
            ## Called at the start of thrower process
            ## Control: Y
            if j.Y():
                if (not pressedY) and (not intakeHasBall) and (not motors.getDIO(THROWER_LIMIT)) and (not throwInProgress):
		...
		motors.setTarget(ARM_INTAKE, MOTOR_OFF) ## Turn off arm intake.
            ...
            ## Control: A
            if j.A():
		if (not pressedA) and (not throwInProgress):
	    ...
	    if throwerConfig:
            ...
            ## Left: Min power
            ## Right: Max Power
            if abs(j.rightY()) > 0.8 and throwerOn and (not throwSpeedLock):
                if pressedRJ == False:
                    if j.rightY() > 0.8 and (throwSpeed + THROWER_INC <= THROWER_MAX):
                        throwSpeed = throwSpeed + THROWER_INC
                        motors.setTarget(THROWER, throwSpeed)   ## Start thrower motor.
                        #print "Increment"
                    if j.rightY() < -0.8 and (throwSpeed - THROWER_INC >= THROWER_MIN):
                        throwSpeed = throwSpeed - THROWER_INC
                        motors.setTarget(THROWER, throwSpeed)   ## Start thrower motor.
                        #print "Decrement"
                    pressedRJ = True
            ...
            if j.rightX() > 0.8 and throwerOn and (not throwSpeedLock):
	    ...
	    if j.leftTrigger() and (not ArmDisabled):
